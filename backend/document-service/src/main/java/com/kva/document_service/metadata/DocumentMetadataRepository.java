...
public class DocumentMetadataRepository {
    ...
    public DocumentMetadata save(DocumentMetadata metadata) {
        String sql = """
            INSERT INTO document_metadata (document_id, product, revision, department, manufacturer, tags, category, effective_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            RETURNING id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at
            """;
    ...
    public Optional<DocumentMetadata> findById(Long id) {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata WHERE id = ?";
        try {
    ...
    public Optional<DocumentMetadata> findByDocumentId(Long documentId) {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata WHERE document_id = ?";
        try {
    ...
    public List<DocumentMetadata> findAll() {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper);
    }

    public List<DocumentMetadata> findByProduct(String product) {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata WHERE product = ? ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, product);
    }

    public List<DocumentMetadata> findByDepartment(String department) {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata WHERE department = ? ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, department);
    }

    public List<DocumentMetadata> findByCategory(String category) {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata WHERE category = ? ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, category);
    }

    public List<DocumentMetadata> findByTag(String tag) {
        String sql = "SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at FROM document_metadata WHERE ? = ANY(tags) ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, rowMapper, tag);
    }

    public List<DocumentMetadata> searchByMetadata(String searchTerm) {
        String sql = """
            SELECT id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at
            FROM document_metadata
            WHERE product ILIKE ? OR revision ILIKE ? OR department ILIKE ? OR manufacturer ILIKE ? OR category ILIKE ?
            ORDER BY created_at DESC
            """;
        String searchPattern = "%" + searchTerm + "%";
        return jdbcTemplate.query(sql, rowMapper, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern);
    }

    public DocumentMetadata update(DocumentMetadata metadata) {
        String sql = """
            UPDATE document_metadata
            SET product = ?, revision = ?, department = ?, manufacturer = ?, tags = ?, category = ?, effective_date = ?
            WHERE id = ?
            RETURNING id, document_id, product, revision, department, manufacturer, tags, category, effective_date, created_at, updated_at
            """;
        return jdbcTemplate.queryForObject(sql, rowMapper,
